...
def get_ego_past_array_from_history_buffer(history_buffer: SimulationHistoryBuffer, num_past_poses):
    ...
    sample_interval = history_buffer.sample_interval
...
def calculate_additional_ego_states(ego_agent_past, time_stamp):
    ...
    # ego_agent_past: (num_past_poses, 7), [x, y, heading, vx, vy, ax, ay]

    current_state = ego_agent_past[-1]
    if len(ego_agent_past) == 1:
        # If there is only one past state, we cannot calculate steering angle and yaw rate
