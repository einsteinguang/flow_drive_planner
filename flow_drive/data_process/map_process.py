...
from flow_drive.data_process.utils import vector_set_coordinates_to_local_frame
...
def map_process(route_roadblock_ids, anchor_ego_state, coords, traffic_light_data, speed_limit, lane_route, map_features, max_elements, max_points):
    ...
    if valid_route_lanes.size == 0:
        ...
        lanes_is_route = np.zeros((vector_map_lanes.shape[0], 1), dtype=np.int64)
        for lane_idx, (lf, ll) in zip(valid_lanes_idx, zip(lane_first_centers, lane_last_centers)):
